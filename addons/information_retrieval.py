...
class GraphRAG(InformationRetrieval):
    ...
    async def route_label(self, query):
        ...
        # outputs = [
        #     RouteItem(label="Trademark", entity="华为"),
        #     RouteItem(label="Category3", entity="手机")
        # ]
    ...
    async def node_retrieval(self, route_res, top_k):
        ...
        for label, query_text, query_vector in zip(labels, query_texts, query_vectors):
            ...
            retriever = HybridRetriever(
                self.driver,
                vector_index_name=label.lower() + "_vector",  # 向量索引名称，对应前边vector_indexing方法，创建索引的名字
                fulltext_index_name=label.lower() + "_fulltext",  # 全文索引名称，对应前边
            )
            tasks.append(

                # 将同步的检索操作包装为异步任务，以支持并发执行
                asyncio.to_thread(
                    retriever.get_search_results,  # 检索方法：混合检索的搜索结果获取方法
                    query_text,                    # 查询文本：用于全文检索的文本
                    query_vector,                  # 查询向量：用于向量检索的向量表示
                    top_k,                         # 返回数量：限制返回的检索结果数量
                    effective_search_ratio=2,      # 搜索比例参数：控制实际搜索的节点数量与top_k的比例
                )
            )
    ...
    async def search(
        ...
        if not query:
            # SearchResultList是rasa的rag找回专用存储器
        ...
        # langchain的graph相关校正工具，校正关系方向。如果某个关系和其反向关系都不合法，会返回空字符串
